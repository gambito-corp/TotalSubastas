...
{{--            <a class="btn btn-primary rounded-pill pr-3 pl-2 btn-to_action-bottom text-light" href="{{ route('auctionLiveDetail', ['id' => \App\Helpers\Gambito::hash($producto->id)])}} "><i class="fas fa-eye pr-3 pl-3 "></i> Participar </a>--}}
        @else
            @unless($estado)
                <button class="btn btn-primary rounded-pill  btn-to_action-bottom text-light" id="send" wire:click="pujar">
...
{{--                <p class="btn btn-success rounded-pill text-light" style="cursor:none"><i class="fas fa-star" wire:click="pujar"></i> Vas Ganando </p>--}}
            @endunless
        @endif
    @else
