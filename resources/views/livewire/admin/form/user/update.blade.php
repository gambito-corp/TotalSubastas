<div>
    <form action="{{route('admin.user.update',['id'=>$identificador])}}" method="Post" enctype="multipart/form-data">
        @csrf @method('PUT')
        <div class="card">
            <div class="card-body">
                ...
                <div class="form-group">
                    <div class="custom-control">
                        <label for="password">Password (Solo Actualizar la nueva contraseña)</label>
                        <input type="password" name="password"  class="form-control  @error('password') is-invalid @enderror" value="{{old('password')}}" wire:model.lazy="password">
                        @error('email')
                        <span class="invalid-feedback" role="alert">
                            <strong>{{ $message }}</strong>
                        </span>
                ...
                <div class="form-group row">
                    ...
                    <div class="custom-control col-md-6">
                        @if (isset($avatar))
                            @include('assets.imagen', ['carpeta' => 'user', 'id' => $identificador, 'ancho' => '150', 'admin' => true])
                        @endif
                    </div>
                </div>
