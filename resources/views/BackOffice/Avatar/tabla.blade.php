...
@endsection
@section('fuentes')
@section('content_header', 'Control de Avatares')
@section('BreadCrumbs', 'Avatares')


@section('contenido')
    <h2>Visualiza Todos Los Avatares {{ request()->routeis('Avatar.index') ? 'Que hay': 'Borrados' }}</h2>
    <a href="{{ route('Avatar.create') }}" class="btn btn-success">Crear un Nuevo Controlador</a>
    @if (request()->routeis('Avatar.index'))        
        <a href="{{ route('Avatar.trash') }}" class="btn btn-info">Ve a la papelera de reciclaje</a>
    @else
        <a href="{{ route('Avatar.index') }}" class="btn btn-info">Ve a la lista de index</a>        
    @endif
    <br><br>
    ...
    <div class="card card-info">
        <div class="card-header">
            <h3 class="card-title">Listado de Avatares {{ request()->routeis('Avatar.index') ? 'Activos': 'Dados de Baja' }}</h3>
        </div>
        <!-- /.card-header -->
        <div class="card-body">            
                ...
                <tbody>                    
                    @forelse ($datos as $data)
                        <tr>
                            ...
                            <td>
                                <a href="{{ route('User.show', ['user' =>  $data->user->username]) }}">{{ $data->user->name }}</a>
                                </td>
                            
                            
                            <td>
                                @if (request()->routeis('Avatar.index'))
                                    <a href="{{route('Avatar.show', ['avatar' => $data->slug])}}">{{ $data->slug }}</a></td>                                
                                @else
                                    {{ $data->slug }}</tr>
                                @endif
                                <td>{{ $data->nombre }}</td>                                
                        ...
                        <div class="modal fade" id="modal-info_{{$data->id}}">
                            <div class="modal-dialog">
                                <div class="modal-content bg-info">
                                    ...
                                    <div class="modal-footer justify-content-between">
                                        <button type="button" class="btn btn-outline-light" data-dismiss="modal">Close</button>
                                        <form action="{{ route('Avatar.edit', ['avatar' => $data->slug]) }}" method="get">
                                             @csrf
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content bg-warning">
                                    ...
                                    <div class="modal-footer justify-content-between">
                                        ...
                                        <form action="{{ route('Avatar.delete', ['avatar' => $data->id]) }}" method="post">
                                            @method('PUT') @csrf
                            ...
                            <div class="modal-dialog">
                                <div class="modal-content bg-info">
                                    ...
                                    <div class="modal-footer justify-content-between">
                                        ...
                                        <form action="{{ route('Avatar.restore', ['id' => $data->slug]) }}" method="post">
                                             @csrf
                        ...
                        <div class="modal fade" id="modal-danger-Confirmar_{{$data->id}}">
                            <div class="modal-dialog">
                                <div class="modal-content bg-danger">
                                    ...
                                    <div class="modal-footer justify-content-between">
                                        <button type="button" class="btn btn-outline-light" data-dismiss="modal">Close</button>
                                        <form action="{{ route('Avatar.destroy', ['avatar' => $data->slug]) }}" method="post">
                                            @csrf @method('DELETE')
