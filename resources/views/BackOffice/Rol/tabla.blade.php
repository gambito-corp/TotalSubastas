...
<table id="example" class="table table-bordered table-striped dataTable dtr-inline" role="grid" aria-describedby="example1_info">
    ...
    <tbody>
        ...
        <tr>
            ...
            <td><a href='{{ route('rol.show', ['rol' => $data->slug]) }}'>{{ $data->nombre }}</a></td>
            ...
            <div class="modal fade" id="modal-info_{{ $data->id }}">
                <div class="modal-dialog">
                    <div class="modal-content bg-info">
                        <div class="modal-header">
                            <h4 class="modal-title">{{ "Actualizar El Rol $data->nombre" }}</h4>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <div class="modal-body">
                            <p>{{ "Deseas Actualizar al Rol $data->nombre" }}</p>
                        </div>
                        <div class="modal-footer justify-content-between">
                            <button type="button" class="btn btn-outline-light" data-dismiss="modal">Cerrar</button>
                            <a href="{{route('rol.edit', ['rol' => $data->slug])}}" class="btn btn-outline-light">Actualizar</a>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <div class="modal fade" id="modal-warning_{{ $data->id }}">
                <div class="modal-dialog">
                    <div class="modal-content bg-warning">
                        <div class="modal-header">
                            <h4 class="modal-title">Dar de Baja al Rol {{ $data->nombre }}</h4>
                            <button type="button" class="close" data-dismiss="modal" aria-label="Close">
                                <span aria-hidden="true">&times;</span>
                            </button>
                        </div>
                        <div class="modal-body">
                            <p>Si das de baja al Rol {{ $data->nombre }} este no podra acceder a la intranet hasta que decidas volver a darlo de alta en la plataforma</p>
                        </div>
                        <div class="modal-footer justify-content-between">
                ...
                <div class="modal-dialog">
                    <div class="modal-content bg-info">
                        ...
                        <div class="modal-footer justify-content-between">
                            ...
                            <form action="{{route('rol.restore', ['id' => $data->id])}}" method="POST">
                                @csrf @method('POST')
            ...
            <div class="modal fade" id="modal-danger_{{ $data->id }}">
                <div class="modal-dialog">
                    <div class="modal-content bg-danger">
                        ...
                        <div class="modal-body">
                            <p>Esto eliminara al Rol {{ $data->nombre }} de forma irrecuperable una vez presionado y confirmado el boton eliminar no hay marcha atras <br> desear eliminar al {{ $data->nombre }}</p>
                        </div>
                        <div class="modal-footer justify-content-between">
                            <button type="button" class="btn btn-outline-light" data-dismiss="modal">Cerrar</button>
            ...
            <div class="modal fade" id="modal-danger-confirmacion_{{ $data->id }}">
                <div class="modal-dialog">
                    <div class="modal-content bg-danger">
                        ...
                        <div class="modal-body">
                            <p>100% Seguro de eliminar al  {{ $data->nombre }}, Podrias generar un fallo en cascada de hacerlo</p>
                        </div>
                        <div class="modal-footer justify-content-between">
                            <button type="button" class="btn btn-outline-light" data-dismiss="modal">Cerrar</button>
                            <form action="{{ route('rol.destroy', $data) }}" method="post">
                                @csrf @method('DELETE')
