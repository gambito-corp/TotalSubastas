...
@if (request()->routeis('Autorizacion.create'))
...
@section('contenido')
    ...
    <div class="card card-info col-md-8 mx-auto">
        <div class="card-header">
            <h3 class="card-title">{{ request()->routeis('Autorizacion.create') ? 'Crea Tu Propio Controlador' : 'Actualiza el Controlador '.$data->nombre }}</h3>
        </div>
        <!-- /.card-header -->
        <div class="card-body">
            <form 
            role="form" 
            method="POST" 
            action="{{ request()->routeis('Autorizacion.create') ? route('Autorizacion.store') : route('Autorizacion.update', ['autorizacion' => $data->id]) }}">
                @if (request()->routeis('Autorizacion.edit')) @method('PATCH') @endif
                @csrf
                <div class="row">
                    @if (request()->routeis('Autorizacion.edit'))
                    ...
                    <div class="col-md-12 mx-auto">
                        <div class="form-group">
                            <input type="submit" class="btn btn-success btn-block" name="crear" value="{{ request()->routeis('Autorizacion.create') ? 'Crear Controlador' : 'Actualiza El Controlador' }}">
                        </div>
                    </div>
                </div>
            </form>
