...
@section('title', 'Creacion de Controladores')
...
@section('content')
    ...
    <div class="container bg-light py-3">
        @if (request()->routeis('Autorizacion.edit'))
            <form id="Rol" method="post" action="{{ route('Autorizacion.update', $data) }}">
                @method('PATCH')
        @else
            <form id="Rol" method="post" action="{{ route('Autorizacion.store') }}">
        @endif
            @csrf
            <div class="messages">
